...
public class Final : MonoBehaviour
{
    ...
    // Imagen que cambia durante el diálogo
    public Image dialogImage; // Asignar desde el inspector
    public Sprite[] imagenesDisponibles; // Cargar todos los sprites que vas a usar
    ...
    void ReadCSV()
    {
        ...
        for (int i = 0; i < validLines.Count; i++)
        {
            ...
            if (fields.Length < 10)
            {
                Debug.LogWarning($"?? Línea {i + 1} tiene solo {fields.Length} columnas: {validLines[i]}");
                Array.Resize(ref fields, 10);
            }
    ...
    public IEnumerator WriteText()
    {
        ...
        while (textoNodo != null)
        {
            ...
            cajaDialogo.text = "";
            // ??? Actualizar imagen si el nodo tiene asignada una
            if (!string.IsNullOrWhiteSpace(myDialogueList.lectura[nodoActual].imagen))
            {
                imagenActual = myDialogueList.lectura[nodoActual].imagen;
                Sprite nuevaImagen = imagenesDisponibles.FirstOrDefault(s => s.name == imagenActual);

                if (nuevaImagen != null)
                {
                    dialogImage.sprite = nuevaImagen;
                    dialogImage.enabled = true;
                }
                else
                {
                    Debug.LogWarning($"?? No se encontró sprite con el nombre {imagenActual}");
                    dialogImage.enabled = false;
                }
            }
            else
            {
                // Si no hay imagen asignada, ocultar
                dialogImage.enabled = false;
            }
            ...
            // ?? Aquí usamos la versión modificada
